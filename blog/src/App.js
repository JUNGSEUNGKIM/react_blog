...
function MyModal(props){
    ...
    const{ color, setPosts, date, likes, posts, selectedPostIndex, setModal } =props;
    const post = posts[selectedPostIndex];
    const datee = date[selectedPostIndex];
    ...
    return (
        <div className='modal' style={{background: props.color}}>
            ...
            {/*<button onClick={() => {*/}
            {/*    let posts_updated = [...props.posts]*/}
            {/*    posts_updated[0] = '여자코트 추천'*/}
            {/*    props.setPosts(posts_updated)*/}
            {/*}*/}
            {/*}>*/}
            {/*    제목수정 테스트*/}
            {/*</button>*/}
